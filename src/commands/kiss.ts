...
const kiss = async (message: Message, args: String[]) => {
  ...
  if (message.mentions.length === 0) {
    ...
    return sendMessage(message.channel, `You want to kiss yourself?!?! Please Mention a user to hug!`)
  }

  const output = new Embed()
    .setTitle(`**Kiss 💋**`)
    .setDescription(`**${message.author.username}** kissed **${user.nick}!**`)
